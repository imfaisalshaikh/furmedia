  ...
  return (
    ...
    >
      ...
      <div className="border-b border-white/10 pb-8 flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div>
          <span className="font-mono text-[10px] tracking-widest text-[#00FF99] uppercase block mb-3">{t("services_subtitle")}</span>
        ...
        <AnimatePresence mode="popLayout">
          {filteredServices.map((service, index) => (
            <motion.div
              ...
              className="group border border-white/[0.07] hover:border-[#00FF99]/40 rounded-2xl p-6 bg-white/[0.012] hover:bg-white/[0.03] flex flex-col justify-between relative overflow-hidden transition-all duration-300"
            >
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="space-y-2">
                  <h3 className="text-xl md:text-2xl font-display font-medium text-white group-hover:text-[#00FF99] transition-colors">
                    {service.name}
                  </h3>
                  <p className="font-mono text-xs text-[#00FF99] italic opacity-90">
                    {service.tagline}
                  </p>
                    ...
                    {service.keyAspects.map((aspect, aspectIdx) => (
                      <li key={aspectIdx} className="flex items-start gap-2.5 text-xs text-[#dcdcdc]">
                        <span className="mt-0.5 p-0.5 rounded-full border border-white/[0.08] bg-black inline-block">
                          <Check className="w-2.5 h-2.5 text-[#00FF99]" />
                        </span>
              ...
              {service.metrics && (
                <div className="mt-8 pt-4 border-t border-white/[0.06] flex items-center justify-between relative z-10">
                  ...
                  <span className="font-mono text-xs text-[#00FF99] font-semibold uppercase tracking-wider bg-[#00FF99]/10 border border-[#00FF99]/20 px-2.5 py-1 rounded-full">
                    {service.metrics}
                  </span>
                </div>
              )}
